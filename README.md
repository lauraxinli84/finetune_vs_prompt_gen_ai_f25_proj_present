...
The stratified sampling function divides shots evenly across classes, then distributes remainder to first classes. With 4 shots and 5 classes, this means one example each from classes 0, 1, 2, and 3.
...
**Note on methodology:** Initial few-shot evaluation used a basic extraction function. After observing high failure rates, an improved extraction function was implemented with multiple fallback strategies (pattern matching, digit search, checking first 100 characters). Even with improved parsing, few-shot still significantly underperformed zero-shot, confirming the core finding.
...
```python
# Fitting 2.5B model on Colab Pro L4
